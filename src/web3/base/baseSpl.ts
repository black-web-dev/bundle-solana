...
export class BaseSpl {
  ...
  async getAllSplMints(owner: web3.PublicKey) {
    const tokenAccounts = await this.connection.getTokenAccountsByOwner(owner, { programId: TOKEN_PROGRAM_ID })
      .catch((getTokenAccountsByOwnerError) => { log({ getTokenAccountsByOwnerError }); return null })
    ...
    const mintAccountInfoes = await getMultipleAccounts(this.connection, tokenAddresses)
    ...
    for (let i = 0; i < tokenAddresses.length; ++i) {
      ...
      if (mintAccountInfo.publicKey.toBase58() != ataInfo.accountState.mint.toBase58()) {
        log("miss match")
      }
      ...
      parseTokenInfo.push({
        ...
        balance: calcDecimalValue(Number(ataInfo.accountState.amount.toString()), mintState.decimals),
        supply: calcDecimalValue(Number(mintState.supply.toString()), mintState.decimals),
      })
    }
